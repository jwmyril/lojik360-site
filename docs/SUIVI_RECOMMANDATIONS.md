...
|---|---|---|---|---|
| A1 | Réparer les liens internes morts : `tutoriels/excel-premiere-analyse.html` pointe sur `../data/transferts_diaspora_2000_2025.csv` (le jeu de données du tutoriel, resté sur atmart.ltd) et sur `../formations.html`. Un tutoriel dont le fichier à télécharger tombe en 404 ne peut pas être suivi | 🔴 | **à faire** | 08/09 : 3 références mortes dans 1 page (audit statique sur 42 pages). Le CSV existe dans `Atmart_website/data/` : le copier ici ou pointer l'URL absolue |
| A2 | Corriger la clé i18n du lien « Datasets » du pied de page d'`index.html` : il porte `data-i18n="nav.podcast"`, donc il s'affiche « Podkas » en kreyòl et « Podcast » en anglais et espagnol | 🔴 | **à faire** | 08/09 : vu au navigateur en kreyòl, pied de page « Solisyon (Atmart) → Podkas » pour un lien vers `atmart.ltd/datasets.html`. Créer `nav.datasets` dans les 3 JSON |
...
| # | Recommandation | Gravité | État | Preuve |
|---|---|---|---|---|
| B1 | Rendre les liens vers les tutoriels sensibles à la langue choisie : un lecteur qui a choisi Kreyòl sur `tutoriels.html` (page entièrement traduite) clique « Kòmanse » et atterrit sur `prompting-ia.html`, en français, alors que `prompting-ia.ht.html` existe. Idem depuis les 3 axes de l'accueil et depuis `lojikkid.html`. Solution : `data-href-ht` / `data-href-en` / `data-href-es` sur chaque carte dont la variante existe, et `i18n.js` réécrit `href` au changement de langue | 🔴 | **à faire** | 08/09 : vécu au navigateur (kreyòl choisi → `prompting-ia.html` en français, titre « Prompting & assistant IA — 10 sessions d'action »). C'est la règle « jamais de page à moitié traduite » prise au niveau du parcours, pas de la page |
| B2 | Retirer le français resté dans `assets/i18n/ht.json` : « 3 axes yo », « Wè 3 axes yo », « Leson gratis, ranje selon 3 axes yo », « kesyon debrief » — le mot *axes* est français, le kreyòl dit *aks* | 🔴 | **à faire** | 08/09 : 4 valeurs sur 130 contiennent `axes` ou `debrief` ; visible au navigateur sous le héros (« Wè 3 axes yo ») |
...
| # | Recommandation | Gravité | État | Preuve |
|---|---|---|---|---|
| C1 | Sortir le jeton GitHub d'`admin.html` de `localStorage` (`lojik_admin_token`) : la page est servie sur la même origine que les tutoriels, qui chargent `cusdis.com/js/cusdis.es.js` — un script tiers. Un script tiers compromis sur cette origine lit le jeton et obtient l'écriture sur le dépôt. Au minimum `sessionStorage` (ou mémoire seule), et un rappel d'expiration courte ; mieux : servir `admin.html` depuis une autre origine | 🔴 | **à faire** | 08/09 : `localStorage.setItem("lojik_admin_token", …)` dans `admin.html` ; 4 pages `pensee-critique*` chargent Cusdis. Le jeton est bien un *fine-grained PAT* limité au dépôt — c'est la seule protection actuelle |
| C2 | Créer `confidentialite.html` (4 langues) et la lier depuis le pied de page et à côté de chaque formulaire : le site collecte des e-mails (newsletter, `/subscribe`), des réponses d'entretien et des informations de carrière (`swot360.html` → Worker → modèle de langue), des commentaires (Cusdis) et des conversations (assistant IA). Aucune page ne dit ce qui est gardé, où, combien de temps, ni comment le retirer | 🔴 | **à faire** | 08/09 : 0 page de confidentialité dans le dépôt ; 0 mention « confidentialité / privacy / konfidansyalite » dans `swot360.html` ; le widget de chat affirme « aucune donnée stockée » sans que rien ne l'écrive pour le visiteur |
...
| C4 | Repointer les fils Cusdis sur ce domaine : `data-page-url="https://atmart.ltd/tutoriels/…"` sur 4 pages — les commentaires sont rattachés à l'ancienne adresse, et Cusdis y renvoie | 🟠 | **à faire** | 08/09 : 24 `data-page-url` sur atmart.ltd dans `pensee-critique.{html,ht,en,es}` |
...
| # | Recommandation | Gravité | État | Preuve |
|---|---|---|---|---|
| F1 | Pousser le thème clair/sombre ou le remiser : 40 fichiers modifiés non commités dans l'arbre de travail, `style.css?v=11` en local contre `?v=1` en ligne. Tant que ça dure, le dépôt et le site racontent deux histoires, et un `git add -A` distrait embarque tout d'un coup | 🟠 | **à faire** | 08/09 : `git status` → 40 ` M`, +1 310 lignes ; en ligne `style.css?v=1` sans bloc thème |
...
| F3 | Versionner tous les scripts inclus (`?v=`) : `tutoriels.html` charge `assets/script.js` sans version alors que `index.html` charge `script.js?v=2` — les navigateurs garderont l'ancien indéfiniment sur une page et pas sur l'autre (piège déjà mordu sur atmart.ltd) | 🟠 | **à faire** | 08/09 : 1 inclusion sans `?v=` (`tutoriels.html`) ; `i18n.js?v=1` partout alors que B1/B3 vont le modifier |
